...
func (r *Reconciler) resolvePlacement(ctx context.Context, app *spec.App) (string, bool, error) {
	...
	// Reachable — when the app's pool is "default" (local is implicitly in
	// "default"): it's the controller's own Docker, so unlike a remote node
	// it's never genuinely "unreachable" the way SSH/agent connectivity to a
	// registered node can be. A failed/timed-out Capacity probe only zeroes
	// out its Cap (Known: false, optimistically treated as fitting any
	// requirement by the scheduler) rather than excluding it — this
	// preserves the single-node non-regression guarantee: an unpinned app
	// with no registered nodes must always still be placeable on local, even
	// if its capacity happens to be momentarily unreadable. The only case
	// local is left out of candidates entirely is if it somehow fails to
	// resolve at all, which never happens in practice.
	if pool == "default" {
